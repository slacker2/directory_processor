...
# Example usage:
#
#   Processor.process_file('/home/user/sample.pdf',
#                           method(:get_text),
#                           {output_dest: '/home/user/output',
#                             archive_dest: '/home/user/archive'})
#   Processor.process_dir('/home/user/myPDFs/')
#                          method(:get_text),
#                          {output_dest: '/home/user/output',
#                            output_dir_dup: true,
#                            archive_dest: '/home/user/archive',
...
module Processor
  ...
  def do_work(file, operation, options, dir = '/')
    ...
    File.open(output_textfile).write(operation_output).close
    archive_file(file, options, dir) unless options[:archive_dest].nil?
    'Success'
  end

  def validate_file_options(file, operation, options)
  ...
  def archive_file(file, options, dir = '/')
    archive_path = archive_file_path(file, dir)
    FileUtils.mkdir_p(archive_path)
    FileUtils.mv(file, archive_path)
  end

  def output_file(file, options = {}, dir = '/')
    file_name = File.basename(test, File.extname(test)) + '.txt'
  ...
  def output_file_path(file, options = {}, dir)
    options[:output_dest].nil? ? File.dirname(file) : options[:output_dest] + file_subpath(file, dir)
  end
